...
async function pingProvider(url: string, timeoutMs: number = 3000): Promise<boolean> {
  ...
  const fastestProvider = await getCached<string | null>(
    ...
    async () => {
      ...
      for (const provider of PROVIDERS) {
        ...
        console.log(`[Stream Fallback] Testing provider: ${provider} at ${testUrl}`);
        const isUp = await pingProvider(testUrl, 3000); // 3-second strict timeout
        
        if (isUp) {
          console.log(`[Stream Fallback] SUCCESS: Provider ${provider} is fully responsive.`);
          return provider;
        }
      }
      return null;
