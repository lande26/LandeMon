...
// ============= FIX #1: Type validator =============
// Converts MediaType.ANIME or invalid types to valid TMDB types
function normalizeMediaType(type: string): 'movie' | 'tv' {
  const lowerType = type.toLowerCase();
  
  // If type contains 'tv', 'series', or 'anime', return 'tv'
  if (lowerType.includes('tv') || lowerType.includes('series') || lowerType === 'anime') {
    return 'tv';
  }
  
  // If type is 'movie', return 'movie'
  if (lowerType === 'movie') {
    return 'movie';
  }
...
class MovieService extends BaseService {
  ...
  // ============= FIX #2: Added type validation and error handling =============
  static findMovieByIdAndType = cache(async (id: number, type: string) => {
    // Validate and normalize the type parameter
    const validType = normalizeMediaType(type);
    ...
    try {
      const response: AxiosResponse<ShowWithGenreAndVideo> = await this.axios(
        baseUrl,
      ).get<ShowWithGenreAndVideo>(`/${validType}/${id}`, { params });
      return Promise.resolve(response.data);
    } catch (error) {
      // If the request fails with one type, try the other
      console.warn(`Failed to fetch ${validType}/${id}, trying alternate type`);
      
      const alternateType = validType === 'movie' ? 'tv' : 'movie';
      
      try {
        const response: AxiosResponse<ShowWithGenreAndVideo> = await this.axios(
          baseUrl,
        ).get<ShowWithGenreAndVideo>(`/${alternateType}/${id}`, { params });
        return Promise.resolve(response.data);
      } catch (secondError) {
        // Both failed, throw the original error
        console.error(`Failed to fetch content with id ${id} as both movie and tv`);
        throw error;
      }
    }
  });
